...
resource "aws_ecrpublic_repository" "aws_ecr_repo_pub" {
  ...
  repository_name = "aws_ecr_repo_pub_batchseven-tf"
...
resource "aws_ecs_task_definition" "flaskapp_task_def" {
  family                   = "flaskapp_task_def_batchseven-tf"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      ...
      image = "public.ecr.aws/c2s9r9y6/aws_ecr_repo_pub_batchseven-tf:latest"
      essential = true
      portMappings = [
        { "protocol"    = "tcp"
          containerPort = 8080
      ...
      logConfiguration = {
        "logDriver" = "awslogs",
        "options" = {
          "awslogs-group"         = "/ecs/flaskapp_task_def_batchseven-tf",
...
resource "aws_ecs_task_definition" "flaskapp_task_defec2" {
  family                = "flaskapp_task_def_batchseven-tfec2"
  cpu    = 512
  memory = 512
